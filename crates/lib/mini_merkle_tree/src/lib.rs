...
const EMPTY_MERKLE_ROOT: &str = "fef7bd9f889811e59e4076a0174087135f080177302763019adaf531257e3a87";
    ...
    H: HashEmptySubtree<LEAF_SIZE>,
{
    ...
    pub fn merkle_root(self) -> B256 {
        if self.hashes.is_empty() {
            B256::from_str(EMPTY_MERKLE_ROOT).unwrap()
        } else {
    ...
    fn compute_merkle_root_and_path(
        self,
        mut index: usize,
        mut merkle_path: Option<&mut Vec<B256>>,
    ) -> B256 {
...
pub trait HashEmptySubtree<const LEAF_SIZE: usize>:
    'static + Send + Sync + Hasher<Hash = B256>
{
...
impl HashEmptySubtree<88> for KeccakHasher {
    fn empty_subtree_hash(&self, depth: usize) -> B256 {
        static EMPTY_TREE_HASHES: Lazy<Vec<B256>> = Lazy::new(compute_empty_tree_hashes::<88>);
