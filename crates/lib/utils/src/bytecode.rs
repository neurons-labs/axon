...
impl CompressedBytecodeInfo {
    ...
    pub fn encode_call(&self) -> Vec<u8> {
        let bytecode_hash = hash_bytecode(&self.original).to_vec();
        let empty_cell = vec![0u8; 32];

        let bytes_encoded = (self.original.clone(), self.compressed.clone()).abi_encode_params();
...
pub fn hash_bytecode(code: &[u8]) -> B256 {
    ...
    let hash = zkvm_opcodes::utils::bytecode_to_code_hash(&chunked_code).expect("Invalid bytecode");

    B256::new(hash)
}

pub fn bytecode_len_in_words(bytecodehash: &B256) -> u16 {
    u16::from_be_bytes([bytecodehash[2], bytecodehash[3]])
}

pub fn bytecode_len_in_bytes(bytecodehash: B256) -> usize {
    bytecode_len_in_words(&bytecodehash) as usize * 32
}

#[cfg(test)]
mod test {
    use axon_primitives::web3::hex;
