...
struct TreeUpdateMetrics {
    ...
    /// Number of keys read while processing a single block (only applicable to the full operation
    /// mode).
    #[metrics(buckets = NODE_COUNT_BUCKETS)]
...
struct ApplyPatchMetrics {
    ...
    /// Total byte size of nodes included into a RocksDB patch per block, grouped by the key nibble
    /// count.
