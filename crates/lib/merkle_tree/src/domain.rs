...
pub struct TreeMetadata {
    ...
    /// State diffs performed in the processed L1 batch sorted by (address, key) as expected by the
    /// circuits/ The information in here is an aggregation of `initial_writes` and
    /// `repeated_writes`.
...
impl AxonTree {
    ...
    pub fn reader(&self) -> AxonTreeReader {
        let db = self.tree.db.inner().clone();
        AxonTreeReader(MerkleTree::new(db))
    }
                ...
                TreeLogEntry::Updated {
                    ...
                    if prev_value_hash != input_entry.value {
                        ...
                        state_diffs.push(StateDiffRecord {
                            ...
                            initial_value: b256_to_u256(prev_value_hash),
                            final_value: b256_to_u256(input_entry.value),
                        });
                    }
...
/// Readonly handle to a [`AxonTree`].
#[derive(Debug)]
pub struct AxonTreeReader(MerkleTree<RocksDBWrapper>);

// While cloning `MerkleTree` is logically unsound, cloning a reader is reasonable since it is
// readonly.
impl Clone for AxonTreeReader {
    fn clone(&self) -> Self {
