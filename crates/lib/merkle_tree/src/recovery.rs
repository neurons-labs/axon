...
impl<DB: PruneDatabase, H: HashTree> MerkleTreeRecovery<DB, H> {
    ...
    /// # Panics
    ///
    /// - Panics if the tree DB exists and it's not being recovered, or if it's being recovered for
    ///   a different tree version.
    /// - Panics if the hasher or basic tree parameters (e.g., the tree depth) do not match those of
