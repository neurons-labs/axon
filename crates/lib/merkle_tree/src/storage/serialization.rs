...
impl LeafNode {
    pub(super) fn deserialize(bytes: &[u8]) -> Result<Self, DeserializeError> {
        ...
        let full_key = Key::from_be_slice(&bytes[..KEY_SIZE]);
    ...
    pub(super) fn serialize(&self, buffer: &mut Vec<u8>) {
        ...
        let key_bytes: [u8; KEY_SIZE] = self.full_key.to_be_bytes();
...
mod tests {
    ...
    fn create_internal_node() -> InternalNode {
        ...
        node.child_ref_mut(1).unwrap().hash = B256::new([1; 32]);
        node.insert_child_ref(0xb, ChildRef::leaf(2));
        node.child_ref_mut(0xb).unwrap().hash = B256::new([11; 32]);
        node
    }
