...
use axon_types::{primitives::hasher::blake2::Blake2Hasher, B256, U256};
use rand::{
    rngs::StdRng,
...
pub(super) const FIRST_KEY: Key = U256::from_limbs([0, 0, 0, 0x_dead_beef_0000_0000]);
const SECOND_KEY: Key = U256::from_limbs([0, 0, 0, 0x_dead_beef_0100_0000]);
const THIRD_KEY: Key = U256::from_limbs([0, 0, 0, 0x_dead_d00d_1234_5678]);
const E_KEY: U256 = U256::from_limbs([0, 0, 0, 0x_e000_0000_0000_0000]);
...
pub(super) fn generate_nodes(version: u64, nibble_counts: &[usize]) -> HashMap<NodeKey, Node> {
    let nodes = nibble_counts.iter().map(|&count| {
        ...
        let node = LeafNode::new(TreeEntry::new(FIRST_KEY, count as u64, B256::ZERO));
        (key, node.into())
    });
    nodes.collect()
}
...
fn assert_storage_with_2_keys(updater: &TreeUpdater) {
    ...
    assert_eq!(leaf.value_hash, B256::new([1; 32]));
    ...
    assert_eq!(leaf.value_hash, B256::new([2; 32]));
}
...
fn assert_storage_with_3_keys(updater: &TreeUpdater) {
    ...
    assert_eq!(leaf.value_hash, B256::new([3; 32]));
}
...
fn reading_keys_does_not_change_child_version() {
    ...
    let storage = Storage::new(&db, &(), 0, true);
    let kvs = vec![
        TreeEntry::new(FIRST_KEY, 1, B256::new([0; 32])),
        TreeEntry::new(SECOND_KEY, 2, B256::new([1; 32])),
    ];
    let (_, patch) = storage.extend(kvs);
    db.apply_patch(patch);
    ...
    let instructions = vec![
        ...
        TreeInstruction::Write(TreeEntry::new(E_KEY, 3, B256::new([2; 32]))),
    ];
...
fn big_endian_key(index: u64) -> U256 {
    U256::from_limbs([0, 0, 0, index.swap_bytes()])
}
...
fn read_instructions_do_not_lead_to_copied_nodes(writes_per_block: u64) {
    ...
    let kvs = (0..key_count)
        .map(|i| TreeEntry::new(big_endian_key(i), i + 1, B256::ZERO))
        .collect();
    let (_, patch) = storage.extend(kvs);
    ...
    for _ in 0..100 {
        ...
        let writes = (key_count..key_count + writes_per_block)
            .map(|i| TreeInstruction::Write(TreeEntry::new(big_endian_key(i), i + 1, B256::ZERO)));
...
fn test_recovery_pruning_equivalence(
    ...
    let entries = (0..100).map(|i| {
        let key = U256::from_limbs([rng.gen(), rng.gen(), rng.gen(), rng.gen()]);
