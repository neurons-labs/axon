...
impl Nibbles {
    ...
    pub fn nibble(key: &Key, index: usize) -> u8 {
        ...
        ((key.as_limbs()[u64_idx] >> shift_in_u64) & 0x0f) as u8
    }

    pub fn new(key: &Key, nibble_count: usize) -> Self {
        debug_assert!(nibble_count <= 2 * KEY_SIZE);
        let mut bytes: [u8; KEY_SIZE] = key.to_be_bytes();
...
mod tests {
    ...
    const TEST_KEY: Key = U256::from_limbs([0, 0, 0, 0x_dead_beef_0000_0000]);
    ...
    fn nibbles_created_from_different_sources_can_be_equal() {
        ...
        let other_key = U256::from_limbs([0, 0, 0, 0x_d000_0000_0000_0000]);
