...
use axon_types::primitives::hasher::{blake2::Blake2Hasher, Hasher};
...
impl dyn HashTree + '_ {
    ...
    fn fold_merkle_path(&self, path: &[ValueHash], entry: TreeEntry) -> ValueHash {
        ...
        for (depth, adjacent_hash) in full_path.enumerate() {
            // if the bit in key of this depth is 1 means the item is on the right.
...
/// No-op hasher that returns `B256::ZERO` for all operations.
impl HashTree for () {
    fn name(&self) -> &'static str {
        "no_op256"
    }
...
#[cfg(test)]
mod tests {
    use axon_types::{AccountTreeId, Address, StorageKey, B256};
    ...
    fn empty_tree_hash_is_as_expected() {
        const EXPECTED_HASH: ValueHash = B256::new([
    ...
    fn leaf_is_hashed_as_expected() {
        ...
        let key = StorageKey::new(AccountTreeId::new(address), B256::ZERO);
        let key = key.hashed_key_u256();
        let leaf = LeafNode::new(TreeEntry::new(key, 1, B256::new([1; 32])));
    ...
    fn folding_merkle_path() {
        ...
        let merkle_path = [B256::new([2; 32]), B256::new([3; 32])];
