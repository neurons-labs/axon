...
use axon_dal::StorageProcessor;
use axon_storage::{db::NamedColumnFamily, RocksDB};
use axon_types::{L1BatchNumber, StorageKey, StorageValue, B256};
use axon_utils::{b256_to_u256, u256_to_b256, U256ONE};
...
impl StateValue {
    pub fn new(value: B256, enum_index: Option<u64>) -> Self {
    ...
    pub fn deserialize(bytes: &[u8]) -> Self {
        if bytes.len() == 32 {
            Self {
                value: B256::from_slice(bytes),
                enum_index: None,
            }
        } else {
            Self {
                value: B256::from_slice(&bytes[..32]),
...
impl RocksdbStorage {
    ...
    async fn apply_storage_logs(
        &mut self,
        storage_logs: HashMap<StorageKey, B256>,
    ...
    async fn save_missing_enum_indices(&self, conn: &mut StorageProcessor<'_>) {
        ...
        let (keys, values): (Vec<_>, Vec<_>) = self
            .db
            .from_iterator_cf(StateKeeperColumnFamily::State, start_from.as_slice())
            .filter_map(|(key, value)| {
                ...
                (state_value.enum_index.is_none())
                    .then(|| (B256::from_slice(&key), state_value.value))
            })
        ...
        let next_key = keys
            .last()
            .and_then(|last_key| b256_to_u256(*last_key).checked_add(U256ONE))
            .map(u256_to_b256);
    ...
    fn process_transaction_logs(
        &self,
        updates: HashMap<StorageKey, B256>,
    ...
    fn store_factory_dep(&mut self, hash: B256, bytecode: Vec<u8>) {
    ...
    pub async fn rollback(
        ...
        tokio::task::spawn_blocking(move || {
            ...
            for factory_dep_hash in &factory_deps {
                batch.delete_cf(cf, factory_dep_hash.as_slice());
            }
    ...
    async fn save(&mut self, l1_batch_number: L1BatchNumber) {
        ...
        let save_task = tokio::task::spawn_blocking(move || {
            ...
            for (hash, value) in pending_patch.factory_deps {
                batch.put_cf(cf, hash.as_slice(), value.as_ref());
            }
            db.write(batch)
                .expect("failed to save state data into rocksdb");
        });
    ...
    fn enum_migration_start_from(&self) -> Option<B256> {
        let value = self
            .db
            .get_cf(
                StateKeeperColumnFamily::State,
        ...
        match value {
            ...
            Some(cursor) => Some(B256::from_slice(&cursor)),
            None => Some(B256::ZERO),
        }
    }
}
...
impl ReadStorage for RocksdbStorage {
    fn read_value(&mut self, key: &StorageKey) -> StorageValue {
        self.read_value_inner(key).unwrap_or(B256::ZERO)
    }
    ...
    fn load_factory_dep(&mut self, hash: B256) -> Option<Vec<u8>> {
        let cf = StateKeeperColumnFamily::FactoryDeps;
        self.db
