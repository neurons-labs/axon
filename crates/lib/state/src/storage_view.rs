...
use axon_types::{witness_block_state::WitnessBlockState, StorageKey, StorageValue, B256};
    ...
    S: ReadStorage + ?Sized,
{
    ...
    fn is_bytecode_known(&mut self, bytecode_hash: &B256) -> bool {
...
#[cfg(test)]
mod test {
    use axon_types::{AccountTreeId, Address, B256};
    use axon_utils::b256_from_low_u64_be;
    ...
    fn test_storage_access() {
        ...
        let key = b256_from_low_u64_be(61);
        let value = b256_from_low_u64_be(73);
        ...
        assert_eq!(default_value, B256::ZERO);

        let prev_value = storage_view.set_value(key, value);
        assert_eq!(prev_value, B256::ZERO);
        ...
        let new_value = b256_from_low_u64_be(74);
        storage_view.set_value(key, new_value);
        assert_eq!(storage_view.read_value(&key), new_value);

        let new_key = StorageKey::new(account, b256_from_low_u64_be(62));
