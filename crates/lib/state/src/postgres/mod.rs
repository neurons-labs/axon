...
use axon_dal::{ConnectionPool, StorageProcessor};
use axon_types::{L1BatchNumber, MiniblockNumber, StorageKey, StorageValue, B256};
...
type FactoryDepsCache = Cache<B256, Vec<u8>>;

impl CacheValue<B256> for Vec<u8> {
...
/// [`StorageValue`] together with a miniblock "timestamp" starting from which it is known to be
/// valid.
///
/// Using timestamped values in [`ValuesCache`] enables using it for past miniblock states. As long
/// as a cached value has a "timestamp" older or equal than the requested miniblock, the value can
/// be used.
///
...
impl CacheValue<B256> for TimestampedStorageValue {
    #[allow(clippy::cast_possible_truncation)] // doesn't happen in practice
    fn cache_weight(&self) -> u32 {
        const WEIGHT: usize = mem::size_of::<TimestampedStorageValue>() + mem::size_of::<B256>();
        // ^ Since values are small in size, we want to account for key sizes as well
...
struct ValuesCacheInner {
    ...
    values: Cache<B256, TimestampedStorageValue>,
}
...
impl PostgresStorageCaches {
    ...
    /// # Panics
    ///
    /// - Panics if the cache wasn't previously configured using
    ///   [`Self::configure_storage_values_cache()`].
