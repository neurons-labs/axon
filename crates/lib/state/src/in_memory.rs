...
use axon_types::{
    ...
    StorageLogKind, StorageValue, B256,
};
use axon_utils::{u256_to_b256, U256ONE};
...
pub struct InMemoryStorage {
    ...
    pub(crate) factory_deps: HashMap<B256, Vec<u8>>,
    last_enum_index_set: u64,
}

impl InMemoryStorage {
    /// Constructs a storage that contains system smart contracts.
    pub fn with_system_contracts(bytecode_hasher: impl Fn(&[u8]) -> B256) -> Self {
    ...
    pub fn with_system_contracts_and_chain_id(
        chain_id: L2ChainId,
        bytecode_hasher: impl Fn(&[u8]) -> B256,
    ) -> Self {
        ...
        let state_without_indices: BTreeMap<_, _> = contracts
            ...
            .flat_map(|contract| {
                ...
                vec![
                    ...
                    StorageLog::new_write_log(is_known_code_key, u256_to_b256(U256ONE)),
                ]
            })
    ...
    pub fn store_factory_dep(&mut self, hash: B256, bytecode: Vec<u8>) {
