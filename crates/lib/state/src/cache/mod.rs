...
mod tests {
    ...
    fn cache_with_zero_capacity() {
        let zero_cache = Cache::<B256, Vec<u8>>::new("test", 0);
        zero_cache.insert(B256::ZERO, vec![1, 2, 3]);
        assert_eq!(zero_cache.get(&B256::ZERO), None);

        // The zero-capacity `MokaBase` cache can actually contain items temporarily!
        let not_quite_zero_cache = MokaBase::<B256, Vec<u8>>::builder()
            .weigher(|_, value| value.cache_weight())
            .max_capacity(0)
            .build();
        not_quite_zero_cache.insert(B256::ZERO, vec![1, 2, 3]);
        assert_eq!(not_quite_zero_cache.get(&B256::ZERO), Some(vec![1, 2, 3]));
        // The item is evicted after the first access.
        assert_eq!(not_quite_zero_cache.get(&B256::ZERO), None);
    }
}
